...
var formatDate = (name, date) => {
	...
	var utcDate = '= ' + moment(date).utc().format('YYYY-MM-DDTHH:MM:ss.SSS') + 'Z';
...
var getOffset = (date, offset) => {
	...
	if (operation === '+') {
		var d1 = moment(date).add(offsetObject);
		var d2 = date;
		deltaDates(d1, d2);
	} else if (operation === '-') {
		var d1 = moment(date).subtract(offsetObject);
		var d2 = date;
		deltaDates(d1, d2);
	}
};
